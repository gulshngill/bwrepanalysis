...
import belicza.andras.bwhf.model.HackDescription;
import belicza.andras.bwhf.model.PlayerActions;
	...
	private static void scanPlayerForHacks( final ReplayHeader replayHeader, final PlayerActions player, final List< HackDescription > hackDescriptionList, final boolean skipLatterActionsOfHackers ) {
		...
		for ( int actionIndex = 0; actionIndex < actionsCount && actionIndex < 25; actionIndex++ )
			if ( playerActions[ actionIndex ].iteration == 5 ) {
				if ( playerActions[ actionIndex ].actionNameIndex != Action.ANI_UNKNOWN && playerActions[ actionIndex ].actionNameIndex != Action.ANI_LEAVE )
		...
		if ( actionsAtIteration5Count > 1 ) {
			...
			for ( int actionIndex = 0; actionIndex < actionsCount && playerActions[ actionIndex ].iteration <= 10; actionIndex++ )
				if ( playerActions[ actionIndex ].iteration == 10 && playerActions[ actionIndex ].actionNameIndex != Action.ANI_LEAVE ) {
		...
		if ( actionsCount >= 10 ) {
			...
			if ( playerActions[ 0 ].actionNameIndex == Action.ANI_SELECT && ( playerActions[ 1 ].actionNameIndex == Action.ANI_TRAIN || playerActions[ 1 ].actionNameIndex == Action.ANI_HATCH )
					&& playerActions[ 0 ].iteration == playerActions[ 9 ].iteration ) {
				boolean isHack = true;
				for ( int i = 2; i < 10; i += 2 ) {
					if ( playerActions[ i ].actionNameIndex != Action.ANI_SELECT || playerActions[ i+1 ].actionNameIndex != Action.ANI_MOVE
			...
			if ( playerActions[ 8 ].actionNameIndex == Action.ANI_SELECT && ( playerActions[ 9 ].actionNameIndex == Action.ANI_TRAIN || playerActions[ 9 ].actionNameIndex == Action.ANI_HATCH )
					&& playerActions[ 0 ].iteration == playerActions[ 9 ].iteration ) {
				boolean isHack = true;
				for ( int i = 0; i < 8; i += 2 ) {
					if ( playerActions[ i ].actionNameIndex != Action.ANI_SELECT || playerActions[ i+1 ].actionNameIndex != Action.ANI_MOVE
		...
		for ( int actionIndex = 0; actionIndex < actionsCount; actionIndex++ ) {
			...
			if ( action.actionNameIndex == Action.ANI_USE_CHEAT && ( replayHeader.gameType != ReplayHeader.GAME_TYPE_MELEE && replayHeader.gameType != ReplayHeader.GAME_TYPE_FFA && replayHeader.gameType != ReplayHeader.GAME_TYPE_UMS ) )
			...
			if ( replayHeader.gameType != ReplayHeader.GAME_TYPE_UMS ) {
				if ( action.actionNameIndex == Action.ANI_ALLY )
				...
				if ( action.actionNameIndex == Action.ANI_VISION )
			...
			if ( action.actionNameIndex == Action.ANI_BUILD && action.parameters != null && action.parameters.length() > 0 && action.parameterBuildingNameIndex != Action.BUILDING_NAME_INDEX_NON_BUILDING ) {
			...
			if ( action.actionNameIndex == Action.ANI_SELECT || action.actionNameIndex == Action.ANI_BWCHART_HACK )
			...
			if ( action.actionNameIndex == Action.ANI_0X33 )
			...
			if ( !foundTerranComsatCancelHack )
				if ( action.actionNameIndex == Action.ANI_CANCEL_TRAIN && action.parameters.equals( "00 00" ) )
					if ( lastAction != null && lastAction.actionNameIndex == Action.ANI_TRAIN
			...
			if ( actionIndex + 3 < actionsCount ) {
				...
				if ( action.parameters != null && actionAhead1.parameters != null && actionAhead2.parameters != null && actionAhead2.parameters != null )
					if ( action.iteration == actionAhead1.iteration && action.iteration == actionAhead2.iteration && action.iteration == actionAhead3.iteration
					  && action.actionNameIndex == Action.ANI_SELECT && actionAhead2.actionNameIndex == Action.ANI_SELECT
					  && ( actionAhead1.actionNameIndex == Action.ANI_MOVE || actionAhead1.actionNameIndex == Action.ANI_ATTACK_MOVE )
			...
			if ( action.actionNameIndex == Action.ANI_BWCHART_HACK && action.parameters.startsWith( "00 15" ) )
				hackDescriptionList.add( new HackDescription( player.playerName, HackDescription.HACK_TYPE_PROTOSS_MONEYHACK, action.iteration ) );
			
			// Zerg moneyhack with cancelling eggs (from Starcraft version 1.15.1)
			if ( action.actionNameIndex == Action.ANI_CANCEL_TRAIN && ( action.parameters.equals( "FE 00" ) || action.parameters.equals( "00 00" ) ) )
				if ( lastAction != null && lastAction.actionNameIndex == Action.ANI_HATCH )
					hackDescriptionList.add( new HackDescription( player.playerName, HackDescription.HACK_TYPE_ZERG_MONEYHACK, action.iteration ) );
			
			// Subunit enqueue hack: subsequent of "some" subunit build actions being equally 120 iterations from each other
			if ( action.actionNameIndex == Action.ANI_BUILD_SUBUNIT ) {
				...
				do {
					...
					if ( testAction.actionNameIndex == Action.ANI_BUILD_SUBUNIT )
			...
			if ( lastIteration == action.iteration && lastAction.actionNameIndex != action.actionNameIndex && action.actionNameIndex != Action.ANI_HOTKEY && action.actionNameIndex != Action.ANI_ALLY && action.actionNameIndex != Action.ANI_VISION )
			...
			if ( actionIndex + 6 < actionsCount && lastAction != null && lastAction.iteration != action.iteration && action.iteration != playerActions[ actionIndex + 3 ].iteration && playerActions[ actionIndex + 3 ].iteration != playerActions[ actionIndex + 6 ].iteration )
				if ( action.actionNameIndex == Action.ANI_SELECT && playerActions[ actionIndex + 2 ].actionNameIndex == Action.ANI_SELECT
						&& playerActions[ actionIndex + 3 ].actionNameIndex == Action.ANI_SELECT && playerActions[ actionIndex + 5 ].actionNameIndex == Action.ANI_SELECT
						...
						&& ( playerActions[ actionIndex + 1 ].actionNameIndex == Action.ANI_TRAIN && playerActions[ actionIndex + 4 ].actionNameIndex == Action.ANI_TRAIN || playerActions[ actionIndex + 1 ].actionNameIndex == Action.ANI_BUILD_SUBUNIT && playerActions[ actionIndex + 4 ].actionNameIndex == Action.ANI_BUILD_SUBUNIT )
					...
					if ( ++autoUnitRequeueCount == 1 ) {
						...
						lastAutoUnitRequeueOccurance = new HackDescription( player.playerName, playerActions[ actionIndex + 1 ].actionNameIndex == Action.ANI_TRAIN ? HackDescription.HACK_TYPE_AUT0_UNIT_REQUEUE : HackDescription.HACK_TYPE_AUT0_SUBUNIT_REQUEUE, action.iteration );
					}
					else {
						...
						hackDescriptionList.add( new HackDescription( player.playerName, playerActions[ actionIndex + 1 ].actionNameIndex == Action.ANI_TRAIN ?HackDescription.HACK_TYPE_AUT0_UNIT_REQUEUE : HackDescription.HACK_TYPE_AUT0_SUBUNIT_REQUEUE, action.iteration ) );
					}
				}
			...
			if ( action.actionNameIndex == Action.ANI_HOTKEY ) {
				try {
